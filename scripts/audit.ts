...
function applyFilters(chain: ChainInfo, opts: FilterOpts): VaultInfo[] {
  ...
  if (opts.category) out = out.filter((v) => displayCat(v) === opts.category);
  return out;
}
...
async function cmdTvl(chains: ChainInfo[], opts: FilterOpts, tableMode: boolean) {
  ...
  let totalTvl = 0;
  let totalVaults = 0;
  const byChain: Array<{ chain: string; chainId: number; tvl: number; vaults: number }> = [];
  const byCategory: Record<string, { tvl: number; count: number }> = {};
  ...
  for (const c of filtered) {
    ...
    for (const v of c.vaults) {
      ...
      if (!byCategory[cat]) byCategory[cat] = { tvl: 0, count: 0 };
      byCategory[cat].tvl += v.tvlUsd;
      byCategory[cat].count++;
    }
    totalTvl += chainTvl;
    totalVaults += c.vaults.length;
    byChain.push({ chain: c.name, chainId: c.chainId, tvl: chainTvl, vaults: c.vaults.length });
  }
  ...
  if (tableMode) {
    ...
    console.log("-".repeat(39));
    for (const [cat, data] of Object.entries(byCategory).sort((a, b) => b[1].tvl - a[1].tvl)) {
...
async function cmdOverlaps(chains: ChainInfo[], opts: FilterOpts, tableMode: boolean) {
  ...
  for (const chain of chains) {
    if (opts.chainId && chain.chainId !== opts.chainId) continue;
    for (const vault of applyFilters(chain, opts)) {
      for (const strat of vault.strategies) {
        if (!strat.overlap) continue;
        overlaps.push({
...
async function cmdVault(chains: ChainInfo[], address: string, tableMode: boolean) {
  ...
  let found: { vault: VaultInfo; chainName: string; chainId: number } | null = null;

  for (const chain of chains) {
    const vault = chain.vaults.find((v) => v.address.toLowerCase() === addr);
    if (vault) {
      found = { vault, chainName: chain.name, chainId: chain.chainId };
      break;
    }
  }
